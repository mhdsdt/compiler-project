...
def build_regex_of_num(start_dfa_node):
    # states
...
def build_regex_of_id_keyword(start_dfa_node):
    # states
...
def build_regex_of_symbol(start_dfa_node):
    # states
    symbols_group_finisher = DFANode(finished=True, token_type=TokenType.Symbol.value)
    symbol_equals_finisher = DFANode(finished=True, token_type=TokenType.Symbol.value)
    ...
    everything_except_slash = RegexEdge().reject('/')
    everything_except_symbols = RegexEdge().accept_digits().accept_white_spaces().accept_comments().accept_alphabets()

    # create dfa
    start_dfa_node.add_next_node(edge=symbols_except_equal_star, next_node=symbols_group_finisher)
    ...
    symbol_assign.add_next_node(edge=symbol_equal_edge, next_node=symbol_equals_finisher)
    symbol_assign.add_next_node(edge=everything_except_symbols, next_node=symbol_assign_star_finisher)
    symbol_star.add_next_node(edge=everything_except_slash, next_node=symbol_assign_star_finisher)


def build_regex_of_whitespace(start_dfa_node):
    # states
...
def build_regex_of_comment(start_dfa_node):
    # states
    # comment_error = NonTokenizableNode(finished=True, roll_back=True, token_type=TokenType.Comment.value)
    comment_start = DFANode(token_type=TokenType.Comment.value)
    comment_middle = DFANode(token_type=TokenType.Comment.value)
    comment_pre_final = DFANode(token_type=TokenType.Comment.value)
...
def get_default_dfa():
    ...
    build_regex_of_num(start_dfa_node)
    build_regex_of_id_keyword(start_dfa_node)
    build_regex_of_symbol(start_dfa_node)
    build_regex_of_whitespace(start_dfa_node)
    build_regex_of_comment(start_dfa_node)
